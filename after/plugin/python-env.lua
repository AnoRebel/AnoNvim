...
local function get_pipenv_envs()
	local envs = {}
	local path = vim.fn.expand("~/.local/share/virtualenvs")
	local venv = vim.fn.trim(vim.fn.system("ls " .. path))
	if venv == "" then
		return envs
	end
	for match in string.gmatch(venv, "[%w*]*[%p%w*]*") do
		local split = vim.fn.split(match, "-")
		envs["Pipenv: " .. split[1]] = table.concat({ path, match, "bin", "python" }, "/")
	end
	return envs
end
...
local function env_handler()
	...
	local options = vim.tbl_extend("force", get_std_envs(), get_pipenv_envs(), get_poetry_envs())
