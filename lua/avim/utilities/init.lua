...
---@field theme_picker avim.utilities.theme_picker
local M = {}
...
M.map = function(mode, keymap, command, opts)
    ...
    if ok then
        if opts.name ~= nil and command == nil then
            ...
            opts.name = nil
            wk.add({ { keymap, group = name }, })
            -- wk.add({ [keymap] = { name = name } })
        else
            if opts.name ~= nil then opts.group = opts.name end
            opts.name = nil
            cmd = { keymap, command, mode = mode }
            for k, v in pairs(opts) do cmd[k] = v end
            wk.add({ cmd, })
            -- wk.add({ [keymap] = { command, opts.desc or "" } }, opts)
        end
    else
        opts.name = nil
        if command ~= nil then
