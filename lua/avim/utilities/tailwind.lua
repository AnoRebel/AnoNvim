...
M.formatter = function(entry)
  if
    vim.tbl_contains({ "nvim_lsp", "lsp" }, entry.source_name)
    or vim.tbl_contains({ "nvim_lsp", "lsp" }, entry.source_id)
  then
    local words = {}
    for word in string.gmatch(entry.label, "[^-]+") do
    ...
    if color_name == "white" or color_name == "black" then
      ...
      entry.kind = string.rep("󱓻", config.color_square_width) -- "X"
      return { entry, hl_group }
    ...
    local tailwindcss_colors = M.TailwindcssColors
