local mappings_tb = require("avim.core.defaults").mappings -- default & user mappings
local isValid_mapping_TB = require("avim.cheatsheet").isValid_mapping_TB
...
-- dofile(vim.g.base46_cache .. "avcheatsheet")
-- Cheatsheet Keymaps
...
return function()
  ...
  local win = require("avim.cheatsheet").getLargestWin()
  ...
  for _, mappings in pairs(_G.avim.mappings) do
    -- for mode, mappings in pairs(mappings_tb) do
    ...
    local opts = mappings[4]
    local card_header = mode == "n" and "Category" or "Category" .. string.format(" ( %s ) ", mode)
    ...
    if opts.desc then
      largest_str = largest_str > #opts.desc + #prettify_Str(lhs) and largest_str or #opts.desc + #prettify_Str(lhs)
    end
    -- end
    -- end
    -- end
  ...
  local hlgroups_types = {
    heading = "Keyword", -- "NvChHeading",
    mapping = "Comment", -- "NvChSection",
    paddingBlock = "Comment", -- "NvChSection",
    asciiHeader = "Type", -- "NvChAsciiHeader",
    emptySpace = "none",
  }
  ...
  require("avim.utils").map({ "n", "v" }, "<ESC>", function()
