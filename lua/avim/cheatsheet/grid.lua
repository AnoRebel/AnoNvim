...
local avcheatsheet = vim.api.nvim_create_namespace("avcheatsheet")
local mappings_tb = require("avim.core.defaults").mappings
local cheatsheet = require("avim.cheatsheet")

-- filter mappings_tb i.e remove tb which have empty fields
for title, val in pairs(_G.avim.mappings) do
  for mode, mappings in pairs(val) do
    if not cheatsheet.isValid_mapping_TB(mappings) then
      _G.avim.mappings[title][mode] = nil
    end
  end
end

-- Cheatsheet Keymaps
require("avim.utils").map(
  { "n", "v" },
  "<leader>kg",
  "<CMD>lua require('avim.cheatsheet.grid')()<CR>",
  { desc = "Cheatsheet(Grid)" }
)
require("avim.utils").map(
  { "n", "v" },
  "<leader>ks",
  "<CMD>lua require('avim.cheatsheet.simple')()<CR>",
  { desc = "Cheatsheet(Simple)" }
)

vim.api.nvim_create_autocmd("BufWinLeave", {
  callback = function()
    if vim.bo.ft == "avcheatsheet" then
      vim.g.avcheatsheet_displayed = false
    end
  end,
})
...
return function()
  ...
  for _, mappings in pairs(_G.avim.mappings) do
    if type(mappings) == "table" then
      for keybind, mappingInfo in pairs(mappings) do
        if mappingInfo[2] then
          column_width = column_width > vim.fn.strdisplaywidth(mappingInfo[2] .. prettify_Str(keybind)) and column_width
  ...
  local highlights_raw = vim.split(vim.api.nvim_exec("filter " .. "Keyword" .. " hi", true), "\n")
  local highlight_groups = {}

  for _, raw_hi in ipairs(highlights_raw) do
    -- table.insert(highlight_groups, string.match(raw_hi, "NvChHead" .. "%S+"))
    table.insert(highlight_groups, string.match(raw_hi, "Keyword" .. "%S+"))
  end
      ...
      if columns[column_i][i] then
        ...
        if vim.tbl_contains(card_headings, columns[column_i][i]) then
          ...
          vim.api.nvim_buf_add_highlight(
            buf,
            avcheatsheet,
            highlight_groups[math.random(1, #highlight_groups)] or "Keyword",
            i + #ascii_header - 1,
  ...
  for i = 0, #ascii_header - 1, 1 do
    ...
    vim.api.nvim_buf_add_highlight(buf, avcheatsheet, "Type", i, 0, -1)
  end
  ...
  vim.opt_local.filetype = "avcheatsheet"
  ...
  vim.g.avcheatsheet_displayed = true

  vim.keymap.set("n", "<ESC>", function()
    require("avim.utils").close_buffer(buf)
