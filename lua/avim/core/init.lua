...
if vim.fn.has("nvim-0.8") ~= 1 then
	vim.notify("Please upgrade your Neovim base installation. AnoNvim requires v0.8+", vim.log.levels.WARN)
...
function M:init(base_dir)
	...
	avim.base_dir = get_avim_base_dir()
	...
	_G.SESSIONDIR = vim.fn.expand(vim.fn.stdpath("state") .. "/sessions/")
	_G.SHADADIR = shadadir
	_G.UNDODIR = undodir

	require("avim.utils").load_env()
	...
	-- require("avim.utils.statuscol")

	-- Misc
	vim.api.nvim_create_user_command("AvimLog", function()
		vim.fn.execute("edit " .. require("avim.core.log").get_path())
	end, { force = true })
	-- Dim buffer text if inactive
	vim.api.nvim_set_hl(avim.bufferDimNSId or 0, "BufDimText", { bg = "NONE", fg = "#222222", sp = "#555555" })
