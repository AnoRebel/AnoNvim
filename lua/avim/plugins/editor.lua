...
utils.map("v", "<", "<cmd><gv", { desc = "Indent Backwards" })
utils.map("v", ">", "<cmd>>gv", { desc = "Indent Forward" })
utils.map("v", "p", "p<cmd>let @+=@0<CR>", { desc = "Paste" })
-- utils.map("v", "<Tab>", ">gv", { desc = "Tab Forward" })
-- utils.map("v", "<S-Tab>", "<gv", { desc = "Tab Backwards" })
utils.map({ "n", "x", "v" }, "<C-s>", "<CMD>w<CR>", { desc = "Save File" })
utils.map({ "n", "x", "v" }, "<C-S-w>", "<cmd>wa<CR>", { desc = "Save All" })
utils.map({ "n", "v" }, "<Esc>", function()
...
utils.map("i", "<C-s>", "<cmd>w<CR>", { desc = "Save File" })
...
-- Package Manager
utils.map({ "n", "v" }, "<leader>p", nil, { name = "󰏖 Packages" })
...
utils.map({ "n", "v" }, "<leader>bt", "<Cmd>e #<CR>", { desc = "Switch Tab" })
...
utils.map({ "n", "v" }, "<leader>bb", "<cmd>BufferLineGoToBuffer -1<CR>", { desc = "Go to Last Visible Tab" })
-- Opt
for i = 1, 9 do
  utils.map({ "n", "v" }, "<C-" .. i .. ">", "<cmd>BufferLineGoToBuffer " .. i .. "<CR>", { desc = "Go to Tab " .. i })
end
---
...
return {
  ...
  { "HiPhish/rainbow-delimiters.nvim", enabled = defaults.features.indent },
  {
    ...
    enabled = defaults.features.comment,
    event = "BufRead",
    dependencies = {
  ...
  },
  {
    ...
    enabled = defaults.features.session,
    lazy = false,
    dependencies = { "nvim-telescope/telescope.nvim" },
    config = function()
      require("persisted").setup({
        save_dir = defaults.options.sessiondir,
        autoload = false,
        autosave = true,
        use_git_branch = true,
  ...
  },
  {
    ...
    enabled = defaults.features.scrollbar,
    event = "VeryLazy",
    opts = {
      ...
      require("arrow").setup({
        ...
        per_buffer_config = {
          satellite = { -- defualt to nil, display arrow index in scrollbar at every update
            enable = defaults.features.scrollbar,
          },
        },
      })
  ...
  },
  {
    ...
    enabled = defaults.features.multicursors,
    event = "VeryLazy",
    dependencies = {
      "smoka7/hydra.nvim",
    },
  ...
  {
    "toppair/peek.nvim",
    enabled = defaults.features.peek_md,
    event = { "VeryLazy" },
