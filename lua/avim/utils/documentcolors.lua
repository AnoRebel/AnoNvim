...
local NAMESPACE = api.nvim_create_namespace("lsp_documentColor")
...
local function create_highlight(rgb_hex, options)
  ...
  if mode == "foreground" then
    api.nvim_command(string.format("highlight %s guifg=#%s", highlight_name, rgb_hex))
  else
    ...
    api.nvim_command(string.format("highlight %s guifg=%s guibg=#%s", highlight_name, fg_color, rgb_hex))
  end
...
local function buf_set_highlights(bufnr, colors, options)
  ...
  for _, color_info in pairs(colors) do
    if color_info ~= nil or type(color_info) ~= 'number' then
      ...
      api.nvim_buf_add_highlight(bufnr, NAMESPACE, highlight_name, line, start_col, end_col)
    end
  end
end
...
function M.buf_attach(bufnr, options)
  ...
  local trigger_update_highlight, timer = require "avim.utils.defer".debounce_trailing(M.update_highlight,
